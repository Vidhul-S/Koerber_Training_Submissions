...
import com.bank.bankApp.dto.ErrorResponeDto;
...
public class BankAppExceptionHandlerController {
    ...
    public ResponseEntity<ErrorResponeDto> handleBankAccountNotFoundException(AccountNotFoundException ex) {
        System.out.println("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");
    ...
    @ExceptionHandler(NoSufficientFundException.class)
    public ResponseEntity<ErrorResponeDto> handleNotSufficientException(NoSufficientFundException ex) {
        ErrorResponeDto errorResponeDto=ErrorResponeDto
                .builder()
                .timeStamp(LocalDateTime.now())
